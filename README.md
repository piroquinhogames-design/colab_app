...
O inicializador solicitará, sem imprimir os valores, a senha do painel, e-mail e senha do MEGA e, opcionalmente, seu token Civitai. O token é utilizado apenas pelo processo do servidor para consultar e baixar modelos; ele não aparece na interface nem é enviado ao navegador. Copie o endereço `trycloudflare.com` exibido no fim do setup e abra-o em qualquer dispositivo. A senha de acesso protege o painel durante a sessão do túnel. O servidor também inclui um adaptador de compatibilidade para a dependência MEGA em runtimes Python 3.12 do Colab. A instalação atualiza somente a matriz direta e testada **Transformers 4.48.3**, **Tokenizers 0.21.0**, **Diffusers 0.32.2**, **Accelerate 1.3.0**, **Hugging Face Hub 0.28.1**, **PEFT 0.17.0** e **PyCryptodome 3.21.0**, usando `--upgrade --no-deps`. Ela não usa `venv`, não executa `pip check` e não instala ou atualiza **PyTorch, CUDA ou dependências indiretas globais do Colab**. Em seguida, confirma somente a GPU e as versões que o painel importa.

> Se você já executou uma versão anterior do inicializador que instalou uma versão nova de PyTorch, use **Ambiente de execução → Desconectar e excluir ambiente de execução** antes de rodar a célula acima. Isso restaura o PyTorch/CUDA padrão e compatível com a T4.
...
Ao concluir um job, o servidor grava uma PNG e um JSON de metadados contendo prompts, seed, parâmetros, LoRAs, data e estado. Antes de carregar uma LoRA, o servidor remove apenas metadados `.alpha` legados que o conversor SDXL do Diffusers rejeita; se a arquitetura do arquivo continuar incompatível, o job falha com uma mensagem específica sem derrubar o worker. Ambos são enviados ao diretório MEGA `IllustriousStudio` por padrão. No envio do job, o app também substitui o manifesto `last_settings.json` no MEGA com o último prompt, negative prompt, seed, parâmetros e LoRAs. Quando uma nova sessão começa, o app reconstrói a galeria ao baixar os JSONs do arquivo remoto, recupera uma PNG do MEGA quando ela não está no cache local e preenche novamente esses últimos controles. Imagens-base de img2img não são persistidas nesse manifesto; envie a imagem-base de novo antes de repetir esse modo. Se um upload falhar por instabilidade temporária, o botão de sincronização manual reenvia as PNGs concluídas pendentes e o manifesto do último prompt antes de reler o histórico remoto. Ao abrir o painel rapidamente, enquanto a conexão MEGA ainda está sendo preparada, o arquivo aparece como **CONECTANDO** e a galeria local continua disponível; quando a conexão termina, o painel atualiza automaticamente o estado, reenvia pendências e recupera o último prompt, sem exigir um clique em **SINCRONIZAR**.
